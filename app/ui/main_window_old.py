...
from .dialogs.background import BackgroundDialog
from .dialogs.batch_test import BatchTestDialog
from .dialogs.login import LoginDialog
from .dialogs.mask_view import MaskViewDialog
...
from ..core.paths import BACKGROUND_PATH, CAPTURES_DIR
from ..core.state import AppState
from ..core.verdict import Level, match_percentages, pick_by_diameter
...
class MainWindow(QWidget):
    def __init__(self):
        ...
        # Diameter measurement: reference frame loaded once, mask window on demand.
        self._background = measure.load_background(BACKGROUND_PATH)
        self._mask_window: MaskViewDialog | None = None
    ...
    def _build_title_bar(self) -> QWidget:
        ...
        actions = {
           ...
           # "Train new model": self._open_train,
            ...
            "Background reference": self._open_background,
    ...
    def _open_background(self) -> None:
        """Set the empty-conveyor reference used by diameter measurement."""
        if not self._require_login("set the background reference", developer_only=True):
            return
        dlg = BackgroundDialog(lambda: self._latest_qimage, self)
    ...
    def _open_batch_test(self) -> None:
        if not self.engine.is_ready:
            ...
            return
        BatchTestDialog(self.engine, self.state, self).exec()
    ...
    def _measure_diameter(self, result):
        ...
        # Terminal diagnostics: background on/off, threshold applied, fit method.
        self._log(
            f"Measurement — background subtraction: "
            f"{'YES' if m.used_background else 'NO (plain threshold fallback)'}"
            f" | threshold: {m.threshold_used} ({cfg['threshold_method']})"
            f" | fit: {m.fit_method}"
        )
        ...
        self._log(
            f"Diameter: {m.diameter_mm:.1f} mm "
            f"({m.diameter_px:.1f} px min-circle, "
            f"{m.diameter_area_px:.1f} px area-equiv, "
            f"threshold {m.threshold_used}"
            f"{' auto' if m.auto_threshold else ' fixed'})"
        )
    ...
    def _fill_diameter(self, m, model_data) -> None:
        """Show the measured diameter under Status, with the model's spec.

        Displays measured mm and, when the model carries one, the expected mm
        so an operator can see agreement at a glance. Returns: None.
        """
        val = self.result_values["Diameter"]
        if m is None or not m.ok:
            val.setText("not measured")
            val.setStyleSheet(
                f"color: {theme.TEXT_MUTED}; font-size: 12px; border: none;"
                " background: transparent;"
            )
            return
        expected = model_data.diameter if model_data else 0.0
        text = (f"{m.diameter_mm:.1f} mm  (spec {expected:.0f})"
                if expected > 0 else f"{m.diameter_mm:.1f} mm")
        val.setText(text)
        val.setStyleSheet(
            f"color: {theme.TEXT_PRIMARY}; font-size: 12px; border: none;"
            " background: transparent;"
        )
